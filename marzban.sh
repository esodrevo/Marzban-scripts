...
install_marzban_script() {
    FETCH_REPO="esodrevo/Marzban-scripts"
...
install_marzban() {
    ...
    # Fetch releases
    FILES_URL_PREFIX="https://raw.githubusercontent.com/esodrevo/Marzban/master"
...
services:
  marzban:
    image: esodrevo/marzban:${marzban_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
...
EOF
        ...
        if [ "$marzban_version" == "latest" ]; then
            yq -i '.services.marzban.image = "esodrevo/marzban:latest"' "$docker_file_path"
        else
            yq -i ".services.marzban.image = \"esodrevo/marzban:${marzban_version}\"" "$docker_file_path"
        fi
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/esodrevo/Marzban/releases"
